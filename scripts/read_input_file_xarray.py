...
def open_file(filename,fileformat,shortgribname='tp'):
    #Takes a filedata dictionary and returns the open file object
    if fileformat in ['grb2','grb']:
        file=xr.open_dataset(filename,engine='cfgrib',filter_by_keys={'shortName':shortgribname})
    elif fileformat in ['netcdf']:
    ...
    if key in file.keys():
        ...
        if ref_dat is not None:
            if not np.allclose(var,ref_dat,thresh):
                ...
                print("Warning: Unexpected ",key," data in file.")
    else:
        data_availble=False
...
def read_obs_file(indata):
    ...
    if centerdat['infile_format'] in ['binaryf4']:
        ...
        if data_available:
            if keys["varkey"] in file.keys():
                var=file[keys["varkey"]].values
                d=file[keys["varkey"]].dims
            else:
                data_availble=False
...
def read_forecast_file(indata):
    ...
    if centerdat["file_dimension_type"]=='4D':
        var_dat=np.ma.masked_all((centerdat['nens'],centerdat['nfcst'],domaindat['nlats'],domaindat['nlons']))
        ...
        var_dat=np.ma.masked_all((centerdat['nens'],domaindat['nlats'],domaindat['nlons']))
        ...
        var_dat=np.ma.masked_all((centerdat['nfcst'],domaindat['nlats'],domaindat['nlons']))
        var_mask=np.full((centerdat['nfcst'],domaindat['nlats'],domaindat['nlons']),False,dtype=bool)
        fcstlead_dat=centerdat['leads']
        fcstlead_mask=np.full((centerdat['nfcst']),False,dtype=bool)
        file_fcstlead_mask=np.full((centerdat['nfcst']),False,dtype=bool)
    elif centerdat['file_dimension_type']=='2D':
        var_dat=np.ma.masked_all((domaindat['nlats'],domaindat['nlons']))
    ...
    if data_available:
        keys=filedat['keys']
        if keys["lonkey"] in file.keys():
            lons=np.array(file[keys["lonkey"]].values)
            if len(lons)==len(lon_dat)*len(lat_dat):
                lons=np.unique(lons)
            if max(lons)>360:
                lons=lons-360
    ...
    #Read in latitude
    if data_available:
        if keys["latkey"] in file.keys():
            lats=np.array(file[keys["latkey"]].values)
            if len(lats)==len(lon_dat)*len(lat_dat):
                lats=np.unique(lats)
            if centerdat["latdir"]=="NtoS":
                lats=lats[::-1]
    ...
    #Read in lead time if there is lead time in file
        if data_available:
            if keys["leadkey"] in file.keys():
                forecast_time=np.array(file[keys["leadkey"]].values)
    ...
    #Read in ensemble member data
        if data_available:
            if centerdat["ensdata"]=="one_variable":
                ...
                if keys["enskey"] in file.keys():
                    ne=len(file[keys["enskey"]].values)
                else:
                    data_availble=False
                ...
                varlist=np.intersect1d(keys["varkeys"],file.keys())
                ne=len(varlist)
        ...
        if data_available:
            if centerdat["ensdata"]=="one_variable": #All data is in the same variable
                ...
                mylist=[]
                for v in varlist:
                   mylist.append(file[v].values)
                var=np.array(mylist)
                d=('ens',)+file[varlist[0]].dims
                keys['enskey']='ens'
        ...
        if data_available:
            ...
            if keys["varkey"] in file.keys():
                ...
                if len(var)==len(lats)*len(lons):
                    latorig=file[keys["latkey"]].values
                    lonorig=file[keys["lonkey"]].values
                    newvar=np.full((len(lats),len(lons)),-9999.99)
                    for (p,la,lo) in zip(var,latorig,lonorig):
                        newvar[lats==la,lons==lo]=p
                    var=newvar
    ...
    var=np.ma.array(var_dat,mask=var_mask)
    var=np.ma.masked_invalid(var)
    var=np.ma.masked_equal(var,centerdat["missingdata"])
    var=np.ma.masked_less(var,centerdat["mindata"])
    var=np.ma.masked_greater(var,centerdat["maxdata"])
...
def read_input_files(center,ingrid,idate=None,sdate=None,edate=None):
    ...
    if (centerdata["type"] == 'forecast_onefile'): #This assumes latitude, longitude, lead time, and ensemble information in a single file
        ...
        var=np.ma.masked_all((centerdata['nens'],centerdata['nfcst'],domaindata['nlats'],domaindata['nlons']))
        for filei,fn in enumerate(filedata): #Loop through files that make up this forecast
            #print(filei)
            #print(fn['filename'])
            indata['filedata']=fn
            outdata=read_forecast_file(indata)
            if centerdata['accumtype'] == 'accum_gfs_mixed': #Adjust accumulations for gfs data
                if centerdata['timestep']==3:
                    if fn['lead_i']%2==0:
                        lastgrid=outdata['var']
                        outdata['var']=outdata['var']/3.0
                    if fn['lead_i']%2==1:
                        outdata['var']=(outdata['var']-lastgrid)/3.0
                if centerdata['timestep']==1:
                    if fn['lead_i']%6==0:
                        lastgrid=outdata['var']
                    else:
                        lastgrid=outdata['var']
                        outdata['var']=outdata['var']-lastgrid
